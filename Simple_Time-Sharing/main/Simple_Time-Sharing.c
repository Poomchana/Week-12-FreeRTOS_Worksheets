...
void simulate_sensor_task(void)
{
    ...
    for (int i = 0; i < 10000; i++) { volatile int dummy = i; }
...
void simulate_processing_task(void)
{
    ...
    for (int i = 0; i < 100000; i++) { volatile int dummy = i * i; }
...
void simulate_actuator_task(void)
{
    ...
    for (int i = 0; i < 50000; i++) { volatile int dummy = i + 100; }
...
void simulate_display_task(void)
{
    ...
    for (int i = 0; i < 20000; i++) { volatile int dummy = i / 2; }
...
void manual_scheduler(void)
{
    ...
    for (int i = 0; i < 1000; i++) { volatile int dummy = i; }
...
// ==================== Demonstrate Time-Sharing Problems ====================
void demonstrate_problems(void)
{
    ESP_LOGI(TAG, "\n=== Demonstrating Time-Sharing Problems ===");
    ESP_LOGI(TAG, "Problem 1: No priority support - Critical task must wait for less important tasks");
    ESP_LOGI(TAG, "Problem 2: Fixed time slice issues - Short tasks waste time, long tasks get interrupted");
    ESP_LOGI(TAG, "Problem 3: Context switching overhead - Time wasted in switching between tasks");
    ESP_LOGI(TAG, "Problem 4: No inter-task communication - Tasks cannot communicate safely");
}
...
void app_main(void)
{
    ...
    while (1) {
        ...
        // Optional: run experiments and demonstration once
        // Uncomment to test
